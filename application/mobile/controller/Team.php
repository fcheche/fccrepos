...
class Team extends MobileBase
{
    ...
    //     $data['time2'] = trim(I('post.time2'));
    //     $data['company'] = trim(I('post.company'));
    //     $data['addtime'] = time();
    //     $return = Db::name('gonghang')->add($data);
    //     if ($return) {
    //         $res['status'] = 1;
    //         $time = trim(I('post.time'));
    //         $time2 = trim(I('post.time2'));
    //         $res['msg'] = "提交成功，请于".$time.$time2."准时到行办理业务";
    //     }else{
    //         $res['status'] = 0;
    //         $res['msg'] = "提交失败，请电话预约";
    //     }
    ...
    //     $count = M('gonghang')->count();
    //     $page = new Page($count,50);
    //     if($count > 0)
    //     {
    //         $goods_list = M('gonghang')->order("addtime desc")->limit($page->firstRow.','.$page->listRows)->select();
