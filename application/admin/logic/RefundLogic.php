...
class RefundLogic extends Model
{
    ...
    function updateRefundGoods($rec_id,$refund_type=0){
        ...
        accountLog($return_goods['user_id'],$this->refund_deposit,$this->refund_integral,'用户申请店铺退款',0,$return_goods['order_id'],$return_goods['order_sn']);
        }
        //在线支付金额退到余额去
        if($refund_type==1 && $this->refund_money>0){
        accountLog($return_goods['user_id'],$this->refund_money,0,'用户申请店铺退款',0,$return_goods['order_id'],$return_goods['order_sn']);
        }

            Db::name('return_goods')->where(['rec_id'=>$rec_id])->save($up_data);//更新退款申请状态
            Db::name('order_goods')->where(['rec_id'=>$rec_id])->save(['is_send'=>3]);//修改订单店铺状态
        ...
        //如果是分销店铺, 从佣金记录表中扣除此店铺产生的佣金: 该笔订单有三条记录(分别对应一二三级佣金)
