...
class GoodsLogic extends Model
{
       ...
       //标库网接口获取数据
    public function bkapi($regNo,$intCls){
      
      $url='http://api.tmkoo.com/info.php?apiKey=TEST_2483982519&apiPassword=Rd1FA35Qkt&regNo='.$regNo.'&intCls='.$intCls;
      $ch = curl_init();
      ...
      return $output;
        ...
        // 整合商标信息
    public function datashangbiao($arr_s_list,$datas){

        if (!empty($arr_s_list['goods'])) {
            // 遍历所有适用项目 并去重 再转换成字符串
            $last_syxm =implode(' ',array_unique(array_column($arr_s_list['goods'], 'goodsName')));
        }

        if (!empty($arr_s_list['goods'])) {
           // 遍历所有类似群组 并去重 再转换成字符串
            $last_lsqz =implode(',',array_unique(array_column($arr_s_list['goods'], 'goodsCode')));
        }

        if (empty($arr_s_list['regNo'])) {
            return 1;
        }
        //整合数据 基础表
        $add['goods_sn'] = "FZS".str_pad(time(),7,"0",STR_PAD_LEFT).rand(10,99);
        $add['goods_xiangmu'] = $last_syxm;
        $add['zhuanli_sn'] = $arr_s_list['regNo'];//专利号
        $add['goods_name'] = $arr_s_list['tmName'];//商标名称
                    
        if ($arr_s_list['tmName']=='图形') {
                $add['zhleixing'] = 5;
        }elseif(preg_match("/^[\x{4e00}-\x{9fa5}]+$/u",$arr_s_list['tmName'])){
                $add['zhleixing'] = 2;
        }elseif(preg_match("/^[A-Za-z]+$/u",$arr_s_list['tmName'])){
                $add['zhleixing'] = 3;
        }else{
                $add['zhleixing'] = 4;
        }

        $add['zl_leixing'] = $arr_s_list['category'];//商标类型
        $add['zcggdate'] = $arr_s_list['regDate'];//注册公告日期
        $add['images'] = 'http://tmpic.tmkoo.com/'.$arr_s_list['tmImg'];
        $add['zhuangtai'] = 'R标（已成功注册）';
        

        if ($datas['price']<10000) {
            $add['shop_price'] = $datas['price']+3000;
        }elseif ($datas['price']>9999 && $datas['price']<15000) {
            $add['shop_price'] = $datas['price']+2000;
        }elseif ($datas['price']>14999 && $datas['price']<20000) {
            $add['shop_price'] = $datas['price']+1500;
        }elseif ($datas['price']>19999) {
            $add['shop_price'] = $datas['price']+1000;
        }else{
            $add['shop_price'] = 0;
        }

        //组合类型
        $add['goods_lsqunzu'] = $last_lsqz;

        //info表
        $add['csggdate'] = $arr_s_list['announcementDate'];//初审公告日期
        $add['csggqh'] = $arr_s_list['announcementIssue'];//初审公告期号
        $add['zcggqh'] = $arr_s_list['regIssue'];//注册公告期号
        $add['sqdate'] = $arr_s_list['appDate'];//申请日期
        $add['sqname'] = $arr_s_list['applicantCn'];//申请人
        $add['sqaddr'] = $arr_s_list['addressCn'];//申请人地址
        $add['dailic'] = $arr_s_list['agent'];//代理公司
        $add['privatedate'] = $arr_s_list['privateDate'];//专用权期限
        $add['sxquyu'] = '国内';//所需区域

        $add['sellername'] = $datas['sellername'];//卖家账号
        $add['guishu'] = $datas['guishu'];//归属人
    ...
    public function goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
                ...
                if ($cat_id) {
                  $sql = "SELECT * FROM  __PREFIX__goods_category WHERE parent_id = '$cat_id' OR id = '$cat_id' OR parent_id_path like '%$cat_id%' ORDER BY parent_id , sort_order ASC";
                }else{
    ...
    /**
     * 获取指定id下的 所有分类      
     * @global type $goods_category 所有店铺分类
    ...
    /**
     * 动态获取店铺属性输入框 根据不同的数据返回不同的输入框类型
     * @param int $goods_id 店铺id
     * @param int $type_id 店铺属性类型id
     */
    ...
    /**
     *  给指定店铺添加属性 或修改属性 更新到 tp_goods_attr
     * @param int $goods_id  店铺id
     * @param int $goods_type  店铺类型id
     */
    ...
    /**
     * 获取指定规格类型下面的所有规格  但不包括规格项 供店铺分类列表页帅选作用
     * @param type $type_id
     * @param type $checked
     */
     ...
     * @return mixed
     */
    function getSortCategory($parent_id)
    {
        ...
        $categoryList =  M("GoodsCategory")->where($where)->cache(true)->getField('id,name,parent_id,level');
        $nameList = array();
    ...
    function savePreSell($data)
    {
        ...
        if (array_key_exists('shut_down', $data) && $data['shut_down'] == 1) {
            if (!empty($data['id'])) {
                ...
                return array('msg' => '未指定需结束的预售店铺', 'status' => 0, 'url' => U('Promotion/pre_sell_list'));
            }
        }
        ...
        if($goods_main['store_count'] < $ext_info['restrict_amount']){
            return array('msg'=>'预售库存不得大于店铺库存','status'=>0, 'url'=>U('Promotion/pre_sell_info', array('id' => $data['id'])));
        }
        if (empty($data['id'])) {
            //添加一个活动的判断
            ...
            if($is_have_goods){
                if($is_have_goods['is_finished'] == 0){
                    return array('msg'=>'该店铺已经参加了预售活动，不能再次参与预售活动','status'=>0, 'url'=>U('Promotion/pre_sell_info', array('id' => $data['id'])));
                }
            }
            if($goods_main['prom_type'] != 0){
                return array('msg'=>'该店铺已经参加了其他活动，不能参与预售活动','status'=>0, 'url'=>U('Promotion/pre_sell_info', array('id' => $data['id'])));
            }
        }
        if(!empty($data['id']) && !empty($data['goods_id'])){
            ...
            if($goods_activity['start_time'] < time() && $goods_activity['goods_id'] != $data['goods_id']){
                return array('msg'=>'预售活动已经开始不能更改预售店铺','status'=>0, 'url'=>U('Promotion/pre_sell_info', array('id' => $data['id'])));
            }
        }
        if (empty($data['id'])) {
            $add_data = array(
            ...
            $goods_save_data['prom_type'] = 4;//预售店铺类型
            M('goods')->where(array('goods_id'=>$data['goods_id']))->save($goods_save_data);
            $r = M('goods_activity')->add($add_data);
            adminLog("管理员添加店铺预售活动 " . $data['goods_name']);
            if ($r !== false) {
                return array('msg'=>'添加预售店铺活动成功','status'=>1, 'url'=>U('Promotion/pre_sell_list'));
            } else {
                return array('msg'=>'添加预售店铺活动失败','status'=>0, 'url'=>U('Promotion/pre_sell_list'));
            }
        } else {
            $save_data = array(
                'act_name' => $data['goods_name'],
            ...
            if ($r !== false) {
                return array('msg'=>'编辑预售店铺活动成功','status'=>1, 'url'=>U('Promotion/pre_sell_list'));
            } else {
                return array('msg'=>'编辑预售店铺活动失败','status'=>0, 'url'=>U('Promotion/pre_sell_list'));
            }
        }
    }
    /**
    ...
    public function afterSave($goods_id)
    {
        ...
        // 店铺货号
        $goods_sn = "FCC".str_pad(time(),7,"0",STR_PAD_LEFT);
        ...
        //@modify by wangqh fix:删除店铺详情的图片(相册图刚好是主图时)删除的图片仍然在相册中显示. 如果主图存物理图片存在才添加到相册 @{
        ...
        if ($goods_item) {
            ...
            foreach ($goods_item as $k => $v) {
                ...
                $v['store_count'] = trim($v['store_count']); // 记录店铺总库存
                $v['sku'] = trim($v['sku']);
                $data = [
                    'goods_id' => $goods_id,
                    'key' => $k,
