...
{    
    ...
    public function addUser($user)
    {
		$user_count = Db::name('users')
				->where(function($query) use ($user){
                    ...
                    if ($user['qq']) {
                        $query->whereOr('qq',$user['qq']);
                    }
                    if ($user['nickname']) {
                        $query->whereOr('nickname',$user['nickname']);
                    }
				})
				->count();
		if ($user_count > 0) {
			return array('status' => -1, 'msg' => '账号|QQ|手机号已存在');
		}
    ...
    {
        $user_count = Db::name('blacklists')
                ->where(function($query) use ($blacklist){
                    
                    if ($blacklist['tel']) {
                        $query->whereOr('tel',$blacklist['tel']);
                    }
                    if ($blacklist['qq']) {
                        $query->whereOr('qq',$blacklist['qq']);
                    }
                   
                })
                ->count();
        if ($user_count > 0) {
            return array('status' => -1, 'msg' => '名单已存在');
        }
        
        $blacklist['add_time'] = time();
        $id = M('blacklists')->add($blacklist);
        if(!$id){
            return array('status'=>-1,'msg'=>'添加失败');
        }else{
