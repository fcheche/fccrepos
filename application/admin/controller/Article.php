...
class Article extends Base {
    ...
    public function articleList(){
        ...
        $res = $Article->where($where)->order('publish_time desc')->page("$p,$size")->select();
    ...
    public function article(){
     ...
     public function shcfList(){
        ...
        $res = $shcf->where($where)->order('shid desc')->page("$p,$size")->select();
        $count = $shcf->where($where)->count();// 查询满足要求的总记录数
        $pager = new Page($count,$size);// 实例化分页类 传入总记录数和每页显示的记录数
        //$page = $pager->show();//分页显示输出
       ...
       // $a=$shcf->getlastsql();
        if($res){
            foreach ($res as $val){
                $val['on_time'] = date('Y-m-d H:i:s',$val['on_time']);                
                $list[] = $val;
            }
        }
        // $this->assign('a',$a);// 赋值数据集
        $this->assign('list',$list);// 赋值数据集
        $this->assign('pager',$pager);// 赋值分页输出        
        return $this->fetch('shcfList');
    }
    public function shcf(){
        $ArticleCat = new ArticleCatLogic();
        $act = I('GET.act','add');
        $info = array();
        $info['on_time'] = time()+3600*24;
        if(I('GET.shid')){
           $shid = I('GET.shid');
           $info = M('shcf')->where('shid='.$shid)->find();
        }
        $cats = $ArticleCat->article_cat_list(0,$info['cat_id']);
        $this->assign('cat_select',$cats);
        $this->assign('act',$act);
        ...
        public function shcfHandle()
    {
        ...
        if ($data['act'] == 'add') {
            ...
            $r = M('shcf')->where('shid='.$data['shid'])->save($data);
        } elseif ($data['act'] == 'del') {
            $r = M('shcf')->where('shid='.$data['shid'])->delete();  
        }
