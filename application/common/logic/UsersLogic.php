...
class UsersLogic extends Model
{
    ...
    public function login($username,$password)
    {
        ...
        $user = Db::name('users')->where("mobile", $username)->whereOr('nickname', $username)->whereOr('email', $username)->find();
        if (!$user) {
     ...
     * @return array
     */
    public function regs($mobile,$password,$password2,$nickname=""){
        ...
        $user_yz = Db::name('users')->where('nickname', $nickname)->whereOr('mobile', $mobile)->find();
        if ($user_yz) {
           return array('status'=>-1,'msg'=>'用户名或者手机号已存在','result'=>'');
        }

        if($password != $password2){
            return array('status'=>-1,'msg'=>'两次密码不一致','result'=>'');
        }

        if($is_validated != 1){
            return array('status'=>-1,'msg'=>'请用手机号或邮箱注册','result'=>'');
        }
        $map['nickname'] = $nickname ? $nickname : $mobile;
     ...
     * @return array
     */
    public function reg($username,$mobile,$password,$password2){
    	$is_validated = 0 ;
    ...
    public function get_withdrawals_log($user_id,$withdrawals_status=''){
        ...
        $sum = M('withdrawals')->where($withdrawals_log_where)->sum('money');
        $Page = new Page($count, 15);
    ...
    public function get_recharge_log($user_id,$pay_status=0){
        ...
        $sum = M('recharge')->where($recharge_log_where)->sum("account");
        $Page = new Page($count, 15);
    ...
    public function get_goods_collect($user_id){
        ...
        //获取我的收藏列表
            ...
            ->field('c.collect_id,c.add_time,g.goods_id,g.goods_name,g.shop_price,g.is_on_sale,g.cat_id,g.goods_sn')
