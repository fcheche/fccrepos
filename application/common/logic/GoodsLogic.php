...
class GoodsLogic extends Model
{
     ...
     * 获取商品列表页帅选属性
     */
    public function get_filter_attr($goods_id_arr = array(), $filter_param, $action, $mode = 0,$id)
    {
        ...
        // 当前分类id 581天猫type_id1 582淘宝type_id2 583京东type_id3 584其他type_id4
        $now_goods_category = M('goods_category')->where('id',$id)->getField('parent_id');       

        if ($now_goods_category==0) {
            $type_id=substr($id, -1);
        }else{
            $type_id=substr($now_goods_category, -1);
        }
       
        $goods_attr = M('goods_attribute')->where('type_id',$type_id)->cache(true)->getField('attr_id,attr_name,attr_values'); // 属性列表
        // 杨宇
        // $goods_attr = M('goods_attr')->where("attr_id != ''")->select();
        ...
        foreach ($goods_attr as $k => $v) {
            ...
            foreach ($attr_values as $ks => $vs) {
                        // 存在的帅选不再显示
                    if (strpos($old_attr, $v['attr_id'] . '_') === 0 || strpos($old_attr, '@' . $v['attr_id'] . '_'))
                        continue;
                    if ($goods_attribute[$v['attr_id']]['attr_index'] == 0)
                        continue;
                    $v['attr_values'] = trim($vs);
                    // 如果同一个属性id 的属性值存储过了 就不再存贮
                    if (in_array($v['attr_id'] . '_' . $v['attr_values'], (array)$attr_value_arr[$v['attr_id']]))
                        continue;
                    $attr_value_arr[$v['attr_id']][] = $v['attr_id'] . '_' . $v['attr_values'];
                    ...
                    // 帅选参数
                    if (!empty($old_attr))
                        $filter_param['attr'] = $old_attr . '@' . $v['attr_id'] . '_' . $v['attr_values'];
                    else
                        $filter_param['attr'] = $v['attr_id'] . '_' . $v['attr_values'];

                    $list_attr[$v['attr_id']]['attr_values'][] = array('key' => $v['attr_id'], 'val' => $v['attr_values'], 'attr_values' => $v['attr_values'], 'href' => U("$action/index", $filter_param, ''));
            }
    ...
    public function get_type_attr($goods_type_arr = array(), $filter_param, $action, $mode = 0)
    {

        $goods_type_arr = implode(',', $goods_type_arr);
        $goods_type_arr = $goods_type_arr ? $goods_type_arr : '0';
        $goods_attr = M('goods_attribute')->where(['type_id'=>['in',$goods_type_arr],'attr_values'=>['<>','']])->select();
        if (empty($goods_attr)) {
            if ($mode == 1) return array();
            return array('status' => 1, 'msg' => '', 'result' => array());
        }
        ...
        foreach ($goods_attr as $k => $v) {
            ...
            $attr_values = explode("\n",$v['attr_values']);
                ...
                foreach ($attr_values as $key => $val1) {
                    $value=trim($val1);
                     $attr_value_arr[$v['attr_id']][] = $v['attr_id'] . '_' . $value;

                $list_attr[$v['attr_id']]['attr_id'] = $v['attr_id'];
                $list_attr[$v['attr_id']]['attr_name'] = $v['attr_name'];

                // 帅选参数
                if (!empty($old_attr))
                    $filter_param['attr'] = $old_attr . '@' . $v['attr_id'] . '_' . $value;
                else
                    $filter_param['attr'] = $v['attr_id'] . '_' . $value;

                $list_attr[$v['attr_id']]['attr_values'][] = array('key' => $v['attr_id'], 'val' => $v['attr_values'],'attr_values' =>$value, 'href' => U("Goods/$action", $filter_param, ''));
                }
     ...
     * @return array
     */
    function get_filter_menu_new($filter_param, $action)
    {
        $menu_list = array();
        // 品牌
        if (!empty($filter_param['brand_id'])) {
            ...
            $brand['href'] = urldecode(U("/Home/$action/index", $tmp, ''));
            $menu_list[] = $brand;
        }
        // 规格
        if (!empty($filter_param['spec'])) {
            ...
            foreach ($spec_group as $k => $v) {
                ...
                $spec_menu['href'] = urldecode(U("/Home/$action/index", $tmp2, ''));
                $menu_list[] = $spec_menu;
            }
        }
        // 属性
        if (!empty($filter_param['attr'])) {
            ...
            foreach ($attr_group as $k => $v) {
                ...
                $attr_menu['href'] = urldecode(U("/Home/$action/index", $tmp2, ''));
                $menu_list[] = $attr_menu;
            }
        }
        // 价格
        if (!empty($filter_param['price'])) {
            ...
            $price_menu['href'] = urldecode(U("/Home/$action/index", $filter_param, ''));
    ...
    public function add_visit_log($user_id,$goods){
        ...
        if($record){
            ...
            $visit = array('user_id'=>$user_id,'goods_id'=>$goods['goods_id'],'visittime'=>time(),'cat_id'=>$goods['cat_id'],'pingtai_id'=>$goods['pingtai_id']);
