...
class SmsLogic 
{
     ...
     * @param unknown $scene
     */
    public function sendSms($scene, $sender, $params, $unique_id=0,$code)
    {
        ...
        if ($sender != '' && check_mobile($sender)) {//如果是正常的手机号码才发送
            try {
                $resp = $this->sendSmsByMysubmail($sender, $smsTemp['sms_sign'], $smsParam, $smsTemp['sms_tpl_code'],$code);
            } catch (\Exception $e) {
    ...
    private function realSendSms($mobile, $smsSign, $smsParam, $templateCode,$code)
    {
     ...
     */
    private function sendSmsByMysubmail($mobile, $smsSign, $smsParam, $templateCode,$code)
    {
            $url='http://msg.fcheche.com/demo/message_send_dx.php?tel='.$mobile.'&code='.$code.'&mains=蜂车车';
            $curl = curl_init();
