 ...
 * @param unknown $scene
 */
function sendSms($scene, $sender, $params,$unique_id=0,$code)
{
    $smsLogic = new \app\common\logic\SmsLogic;
    return $smsLogic->sendSms($scene, $sender, $params, $unique_id,$code);
}
 ...
 * 发送短信模板
 */
function send_msg($tel,$content){
   
   $url='http://msg.fcheche.com/demo/message_send_word.php?tel='.$tel.'&content='.$content;
            $curl = curl_init();
...
function confirm_order($id,$user_id = 0){
    ...
    if($order['order_status'] != 1)
        return array('status'=>-1,'msg'=>'该订单不能确认完成');
    if(empty($order['pay_time']) || $order['pay_status'] != 1){
        return array('status'=>-1,'msg'=>'商家未确定付款，该订单暂不能确定完成');
    }
